...
const { author, publisher, admin, user, editor,
...
describe('Documents', () => {
  ...
  describe('Fetching all documents', () => {
    ...
    it('should return correct documents(s) for a query', (done) => {
      chai.request(app)
        .get('/api/v1/documents?q=Non')
        .set({ 'x-access-token': adminToken })
        .end((err, res) => {
          expect(res.status).to.equal(200);
          expect(res.body.documents).to.be.a('array');
          expect(res.body.documents[0].title).to.eql('Non-Sucking Document Management System');
          done();
        });
    });
    ...
    it('can limit the number of documents returned', (done) => {
      chai.request(app)
        ...
        .end((err, res) => {
          ...
          dummyId = res.body.documents[1].id;
          done();
        });
    });
    ...
    it('can offset the starting position of returned documents', (done) => {
      chai.request(app)
        .get('/api/v1/documents?offset=1')
        .set({ 'x-access-token': adminToken })
        .end((err, res) => {
          expect(res.status).to.equal(200);
          expect(res.body.documents).to.be.a('array');
          expect(res.body.documents[0].id).to.eql(dummyId);
          done();
        });
    });
  });
    ...
    describe('Fetching documents based on user id', () => {
      ...
      (done) => {
        chai.request(app)
        .get(`/api/v1/documents/${privateDoc.docId}`)
        .set({ 'x-access-token': authorToken })
        .end((err, res) => {
      ...
      (done) => {
        chai.request(app)
        .get(`/api/v1/documents/${roleDoc.docId}`)
      ...
      (done) => {
        chai.request(app)
        .get('/api/v1/documents/3000000000')
        .set({ 'x-access-token': adminToken })
        .end((err, res) => {
          expect(res.status).to.equal(400);
          expect(res.body).to.be.a('object');
  ...
  describe('Deleting a document', () => {
    ...
    (done) => {
      chai.request(app)
      .delete(`/api/v1/documents/${roleDoc.docId}`)
      .set({ 'x-access-token': userToken })
      .end((err, res) => {
        expect(res.status).to.equal(403);
        expect(res.body).to.be.a('object');
    ...
    it('should allow a user to delete his/her document', (done) => {
      chai.request(app)
      .delete(`/api/v1/documents/${privateDoc.docId}`)
      .set({ 'x-access-token': userToken })
      .end((err, res) => {
        expect(res.status).to.equal(200);
        expect(res.body).to.be.a('object');
    ...
    (done) => {
      chai.request(app)
      .delete('/api/v1/documents/3000000000')
      .set({ 'x-access-token': adminToken })
      .end((err, res) => {
        expect(res.status).to.equal(400);
        expect(res.body).to.be.a('object');
